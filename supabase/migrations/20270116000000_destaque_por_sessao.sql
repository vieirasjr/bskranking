-- ============================================================
-- Destaque agora é POR SESSÃO, não por semana calendário.
--
-- A função get_weekly_highlight é mantida (nome mantido por compat
-- com clientes antigos), mas passa a:
--   1. Filtrar stat_logs pelo intervalo [session.started_at .. now()]
--      quando existe sessão ativa naquele location (is_started = true).
--   2. Retornar vazio quando não há sessão ativa → card "Destaque"
--      some entre sessões e reseta quando uma nova começa.
--   3. Descontar tentativas erradas da eficiência:
--      eff = acertos - (shot_1_miss*0.4 + shot_2_miss*0.8
--                      + shot_3_miss*1.2 + turnovers*1.0)
--
-- As colunas de retorno week_start/week_end continuam existindo por
-- compat; passam a carregar session_started_at .. now().
-- ============================================================
...
AS $$
  WITH session_bounds AS (
    -- Bordas da sessão ATIVA no local. Se não há sessão ativa,
    -- session_bounds fica vazio → toda a consulta retorna 0 linhas.
    SELECT started_at AS start_ts, now() AS end_ts
    FROM public.session
